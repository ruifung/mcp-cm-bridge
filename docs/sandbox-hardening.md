...
Status of sandbox hardening measures across all three executor implementations.
...
Five hardening measures were identified in the [isolated-vm sandbox audit](./isolated-vm.md#hardening-recommendations). All applicable measures have been implemented consistently across all three executors.
...
| Test | vm2 | isolated-vm | container |
|------|-----|-------------|-----------|
| `should block prototype pollution` | Runs | Runs | Skipped (no prototype freezing; OS isolation) |
| `should not allow eval` | Runs | Runs | Skipped (eval available inside container; OS isolation) |
| `should not allow constructor to escape` | Runs | Runs | Skipped (Function available inside container; OS isolation) |
| `should not allow access to require` | Runs | Runs | Skipped (require available inside container; OS isolation) |
| `should not allow process access` | Runs | Runs | Skipped (process available inside container; OS isolation) |
| `should isolate data between concurrent executions` | Runs | Skipped (`Object.seal(globalThis)` prevents adding new globals) | Skipped (serialized execution) |

---

## Security Model Summary

| Layer | vm2 | isolated-vm | container |
|-------|-----|-------------|-----------|
| Isolation level | JS-level (vm2 sandbox) | V8 isolate (separate heap) | OS-level (container boundary) |
| Prototype freeze | Yes | Yes | No (not needed) |
| eval blocked | Yes | Yes | Yes |
| Function constructor blocked | Yes | Yes | Yes |
| codemode non-configurable | Yes | Yes | Yes |
| globalThis sealed | No (proxy limitation) | Yes | No (breaks runtime) |
| Internals hidden | N/A (no internal state) | Yes (non-enumerable) | N/A (no internal state on globalThis) |
| Network blocked | No require/fetch available | No network APIs available | `--network=none` at container level |
| Fresh context per call | Yes (new VM) | Yes (new context) | Yes (new worker thread) |
